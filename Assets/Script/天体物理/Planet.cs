...
public class Planet : Attractor
{
    ...
    void Awake()
    {
        itemsInForce = new HashSet<IAttractable>();
    ...
    private void DetectIn()
    {
        ...
        foreach (Collider2D collider in itemsDetected)
        {
            if (collider.gameObject == this.gameObject)
                continue;
            IAttractable attractedItem = collider.GetComponent<IAttractable>();
            if (!itemsInForce.Contains(attractedItem) && attractedItem != null)
            {
                itemsInForce.Add(collider.GetComponent<IAttractable>());
                collider.GetComponent<IAttractable>()?.OnAttract(this);
                //OnAttract回调
            }
    ...
    override protected void ApplyForce()
    {
        ...
        foreach (IAttractable attractedItem in itemsInForce)
        {
